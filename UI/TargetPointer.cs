...
internal class EnhancedTargetPointersUI : UIElement
{
    ...
    /// </summary>
    public static void CreatePointerAndGlowTextures()
    {
        if (_inited) return;
        _inited = true;
    ...
    public override void Draw(SpriteBatch sb)
    {
        ...
        var mouseIdx = _pointers.FindIndex(p => p.IsMouse);
        var glowSet = new HashSet<int>();
        if (mouseIdx >= 0)
        {
            var thr2 = cfg.CloseDistanceThreshold * cfg.CloseDistanceThreshold;
        ...
        if (glowSet.Count > 0)
        {
            ...
            var scaleG = scale * cfg.GlowScaleMultiplier;
        ...
        for (var i = 0; i < _pointers.Count; i++)
        {
            ...
            else
            {
                ...
                var headIdx = p.BossNpc.GetBossHeadTextureIndex();
                var useIcon = cfg.ShowBossIcons && headIdx >= 0;
                if (useIcon)
    ...
    {
        var boss = p.BossNpc;
        var idx = boss.GetBossHeadTextureIndex();
        var headTex = idx >= 0 && idx < TextureAssets.NpcHeadBoss.Length
            ? TextureAssets.NpcHeadBoss[idx].Value
            : TextureAssets.Projectile[ProjectileID.ShadowBeamFriendly].Value;
